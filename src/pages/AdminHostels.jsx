...
function AdminHostels() {
  ...
  const loadHostels = () => {
    const stored = JSON.parse(localStorage.getItem("hostels")) || [];
    setHostels(stored);
  };
  ...
  const saveHostel = () => {
    ...
    const updated = editingId
      ? stored.map((h) => (h.id === editingId ? hostelObj : h))
      : [hostelObj, ...stored];
  ...
  const confirmDelete = () => {
    const stored = JSON.parse(localStorage.getItem("hostels")) || [];
    const updated = stored.filter((h) => h.id !== deleteId);
    localStorage.setItem("hostels", JSON.stringify(updated));
    setDeleteId(null);
    loadHostels();
  };
      ...
      <div className="ah-list">
        ...
        {hostels.length === 0 ? (
          ...
          <div className="ah-grid-list">
            {hostels.map((h) => (
              <div key={h.id} className="ah-hostel">
                ...
                <div className="ah-row-actions">
                  ...
                  <button className="btn-small danger" onClick={() => setDeleteId(h.id)}>
                    Delete
                  </button>
                </div>
              </div>
