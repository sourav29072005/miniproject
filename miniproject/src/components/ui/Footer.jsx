...
const Footer = () => {
  ...
  const [modalType, setModalType] = useState(null); // 'help' | 'safety' | 'terms' | 'privacy'

  return (
    <>
      <footer className="bg-gray-900 text-gray-300 py-12 md:py-16 w-full mt-auto">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 grid grid-cols-1 md:grid-cols-4 gap-12 lg:gap-16">
          ...
          <div>
            ...
            <ul className="space-y-3 font-semibold text-sm">
              <li><button onClick={() => setModalType("help")} className="text-gray-400 hover:text-white transition-colors cursor-pointer">Help Center</button></li>
              <li><button onClick={() => setModalType("safety")} className="text-gray-400 hover:text-white transition-colors cursor-pointer">Safety Guidelines</button></li>
              <li><button onClick={() => setModalType("terms")} className="text-gray-400 hover:text-white transition-colors cursor-pointer">Terms of Service</button></li>
              <li><button onClick={() => setModalType("privacy")} className="text-gray-400 hover:text-white transition-colors cursor-pointer">Privacy Policy</button></li>
            </ul>
          </div>
        </div>
      ...
      <FooterInfoModal type={modalType} onClose={() => setModalType(null)} />
    </>
  );
};
